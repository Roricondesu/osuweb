...
export abstract class GameEngine {
   ...
   *  当 storyboard 视频精灵真正加载到资源时让位，否则作为兜底播放。 */
  protected get useBackgroundVideo(): boolean {
    return !!this.videoElement && this.showVideo && !(this.showStoryboard && this.storyboardVideoAvailable);
  }
  ...
  private loadStoryboardVideos(): void {
    ...
    this.storyboardVideoAvailable = anyAvailable;
  }
  ...
  protected drawProgressBar(): void {
    ...
    if (Number.isFinite(duration) && duration > 0) {
      ratio = clamp(current / duration, 0, 1);
    } else {
      // fallback：用 beatmap 首尾 note 时间跨度
      const objs = this.beatmap.hitObjects;
      if (objs.length > 0) {
        const first = objs[0].time;
        const last = objs[objs.length - 1].endTime || objs[objs.length - 1].time;
        const span = last - first;
        if (span > 0) ratio = clamp((this.currentTime - first) / span, 0, 1);
      }
    }
    const barH = 3;
